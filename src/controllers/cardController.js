...
const hexadecimalColorSchema = Joi.string().pattern(new RegExp('^#([0-9a-fA-F]{3}){1,2}$')).message("\"color\" with value \"#FF00FFF\" failed to match expected format. Please use a valid hexadecimal color.");
...
export async function deleteCard(req, res) {
  ...
  if (!card) { // Si pas entier ou pas existant dans la BDD => 404
    return res.status(404).json({ error: `Card not found` });
  }
...
// TODO : - MAIS ALORS, comment je fais si je veux RETIRER un champs, etc: retirer la couleur
export async function updateCard(req, res) {
  ...
  // Si changement de liste demandé, vérifier que la liste dans laquelle on met la carte existe
  if (list_id) {
    const list = await List.findByPk(list_id);
    if (! list) {
